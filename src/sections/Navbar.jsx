  ...
  return (
    <>
        ...
        <div className='flex flex-col text-5xl gap-y-2 md:text-6xl lg:text-8xl'>
            {["home","services","about","works","contact"].map((section, index) =>(
                <div key={index} ref={(el)=>(linksRef.current[index] = el)}>
                    <Link className='transition-all duration-300 cursor-pointer hover:text-white' to={`${section}`} smooth offset={0} duration={1500}>{section}</Link>
                </div>
            ))}
        </div>
        ...
        <div ref={contactRef} className='flex flex-col flex-wrap justify-between gap-8 md:flex-row'>
            <div className='font-light'>
                ...
                <p className='text-xl tracking-widest lowercase text-pretty'>kithsandurathnayake@gmail.com</p>
            </div>
            ...
            <div className='font-light'>
                ...
                <div className='flex flex-col flex-wrap md:flex-row gap-x-2'>
                    {socials.map((social, index) =>(
                        <a key={index} href={social.href} className='text-sm leading-loose tracking-widest lowercase hover:text-white transition-colors duration-300'>{"{ "}{social.name}{" }"}</a>
                    ))}
                </div>
            </div>
